...
<Callout>
    The order in which you define the bindings is very important.
</Callout>
...
binds all the standard interfaces used internally by `libmodulor` Web implementations.
...
As an example, let's say you are writing a product that allow people to call a LLM.

Typically, in your app you would have a `LLMManager` interface.

Internally, you would have multiple implementations : `OpenAILLMManager`, `MistralAILLMManager`, etc.
...
Anything that needs settings, provides an interface that extends `Settings`.
Here is an example.
...
Typically, a product setting looks like this.
...
As an alternative, you can also provide an implementation named `VaultSettingsManager` that implements `SettingsManager`.
This implementation fetches the secrets from [Vault by Hashicorp](https://www.vaultproject.io) instead.

Again, `libmodulor` provides basics, but you can extend it however your want.
