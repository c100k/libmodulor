...
<Callout>A project can contain multiple apps and products, even if they are not related.</Callout>

The generated directory has the following structure :

```sh
📁 src
    📁 apps # contains the apps of the project (empty for now)
    📁 products # contains the products of the project (empty for now)
